...
namespace AGSUnpacker.Lib.Translation
{
    ...
    public class AGSTranslation
    {
        ...
        public Encoding OriginalEncoding { get; set; } = Encoding.Latin1;
        public Encoding TranslateEncoding { get; set; } = Encoding.Latin1;
                ...
                case BlockType.Content:
                    {
                        for (int i = 0; i < OriginalLines.Count; ++i)
                        {
                            ...
                            writer.WriteEncryptedCString(OriginalLines[i], OriginalEncoding);
                            writer.WriteEncryptedCString(TranslatedLines[i], TranslateEncoding);
                        }
        ...
        public void Decompile(string filepath)
        {
            ...
            {
                using (BinaryReader reader = new BinaryReader(stream, OriginalEncoding))
                {
        ...
        public static AGSTranslation ReadSourceFile(string filename)
        {
            ...
            {
                using (StreamReader reader = new StreamReader(stream, translation.OriginalEncoding))
                {
        ...
        public void WriteSourceFile(string filepath)
        {
            using (FileStream stream = new FileStream(filepath, FileMode.Create, FileAccess.Write))
            {
                using (StreamWriter writer = new StreamWriter(stream, OriginalEncoding))
                {
